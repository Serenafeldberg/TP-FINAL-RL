"""Script principal de entrenamiento PPO para Skiing.
...
def build_env(env_id: str, args=None):
    ...
    if "render_mode" not in env_kwargs and args is not None and args.render:
...
def train(env, args):
    ...
    # Crear Actor-Critic
    print(f"\n[2/5] Creando redes Actor-Critic...")
    ...
    for timestep in range(1, total_timesteps + 1):
        ...
        if timestep % Config.SAVE_FREQ == 0:
            save_path = os.path.join(Config.MODEL_DIR, f"ppo_skiing_{timestep}.pth")
            agent.save(save_path)
    
    # Guardar modelo final
    final_path = os.path.join(Config.MODEL_DIR, "ppo_skiing_final.pth")
